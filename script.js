...
function buildForecastCards(sym) {
    ...
    for (let i = 1; i < dailyForecastData.time.length; i++) {
        ...
        card.addEventListener('click', () => {
            ...
            // NEW: Click Event on Day Card to change background
           updateBackground(code, maxTemp, 1);
        });
...
// ---------------- SPLASH SCREEN LOGIC ----------------
window.addEventListener('load', () => {
    // 3.5 seconds (3500 milliseconds) ka timer set kiya hai
    setTimeout(() => {
        const splash = document.getElementById('splashScreen');
        // Fade out animation trigger hogi
        splash.classList.add('splash-hidden');
        
        // Memory free karne ke liye HTML se completely remove kar dein 1 sec baad
        setTimeout(() => {
            splash.remove();
        }, 1000);
    }, 3500); 
